...
currency_dateBegin = col1.date_input('Ngày bắt đầu', dt.datetime(2021, 6, 1))
currency_dateEnd = col1.date_input('Ngày Kết thúc', dt.datetime(2021, 7, 1))

scale = MinMaxScaler(feature_range=(0,1))
    
model_selected = train_init_model(ticket_selected, currency_type_uint, scale)
test_inputs = create_data(scale, ticket_selected, currency_interval, currency_dateBegin, currency_dateEnd)

predictions = prediction_price(scale, test_inputs, model_selected)
predictions = pd.DataFrame(data=predictions, columns=['Close'])
valid = load_data_predics(ticket_selected, currency_interval,currency_dateBegin, currency_dateEnd)
...
valid_total = valid_total.filter([convert_type_prediction.get(currency_type_uint)])
valid = valid_total[len(valid_total) - len(valid):]
valid['Prediction'] = np.array(predictions[convert_type_prediction.get(currency_type_uint)])
st.markdown("""
